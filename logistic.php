			...
			<div class="logistics-box cf">
				<form id="product_form" method="post">
					<div class="form-header">
						<h4>Logistics (IDF <?php echo $session_location; ?>)</h4>
					</div>
					<div class="grid">
						<div class="col m-1-2">	
					...
					<div class="form-footer grid">
						<div class="col m-1-2">	
							<input type="button" class="button primary" id="receive_product" value="Receive" disabled>
						</div>
						<div class="col m-1-2">	
							<input type="button" class="button secondary" id="send_product" value="Send" disabled>
						</div>
					</div>
				</form>
			</div>
					...
					success: function(d) {
						if(d != "Product not found!" && d != "No such model!") {
							...
							if(d == 'Serial and ModelID do not match!') {
								...
								obj.deactivateBtn();
								console.log('model doesnot match');
							} else {
								//product found
								var data = JSON.parse(d);
								if(obj.idf == '4') {
									console.log('Already present and IDF4. ' + 'Stage: ' + data.ProductStage);
									// product status
									switch(data.ProductStage) {
										case "1":
											obj.updateMessage('Product dispatched from IDF' + data.idfID + ' production. Not yet dispatched from IDF' + data.idfID + ' logistic.', 'warn');
											obj.deactivateBtn();
										break;
										case "1.5":
											obj.updateMessage('Product dispatched from IDF' + data.idfID + ' production. Not yet dispatched from IDF' + data.idfID + ' logistics. Can\'t proceed.', 'warn');
											obj.deactivateBtn();
										break;
										case "1.8":
											obj.updateMessage('Product dispatched from IDF' + data.idfID + ' production. Not yet dispatched from IDF' + data.idfID + ' logistics. Can\'t proceed.', 'warn');
										...
										break;
										case "3":
											obj.updateMessage('Product dispatched from IDF' + data.idfID + ' logistics. Ready to be received.');
											$(obj.sel.receiveBtn).val('Receive from IDF' + data.idfID + ' logistics');
											obj.activateBtn('receive');
											obj.updateData('stage=4');
										break;
										case "4":
											obj.updateMessage('Product received at IDF4 logistics. Ready to be dispatched.');
											$(obj.sel.sendBtn).val('Send to IDF4 production');
											obj.activateBtn('send');
											obj.updateData('stage=5');
										break;
										case "5":
											obj.updateMessage('Product already dispatched from IDF4 logistics! Waiting to be received at ID4 production.', 'warn');
											obj.deactivateBtn();
										break;
										case "6":
											obj.updateMessage('Product received at ID4 production! Waiting to be dispatched from ID4 production.', 'warn');
											obj.deactivateBtn();
										break;
										case "7":
										...
										break;
										case "9":
											obj.updateMessage('Product dispatched from Test Infrastructure after testing. Waiting to be received at ID4 production.', 'warn');
											obj.deactivateBtn();
										break;
										case "10":
									...
									if(data.idfID == obj.idf) {
										if(data.idfID == '1' || data.idfID == '8') {
											switch(data.ProductStage) {
												case "1":
													obj.updateMessage('Product dispatched from IDF' + data.idfID + ' production. Partial testing not yet done. If product does not require partial testing, then skip test and receive product.', 'warn');
													$(obj.sel.receiveBtn).val('Skip IDF' + data.idfID + ' Partial Test & Receive');
													obj.activateBtn('receive');
													obj.updateData('stage=2');
												break;
												case "1.5":
													obj.updateMessage('Product dispatched from IDF' + data.idfID + ' production. Partial testing not yet done.', 'warn');
													obj.deactivateBtn();
												break;
												case "1.8":
													obj.updateMessage('Product dispatched from IDF' + data.idfID + ' production and partial testing done. Ready to be received.');
													$(obj.sel.receiveBtn).val('Receive from IDF' + data.idfID + ' testing');
													obj.activateBtn('receive');
													obj.updateData('stage=2');
												break;
												case "2":
													obj.updateMessage('Product received at IDF' + data.idfID + ' logistic. Ready to be dispatched.');
													$(obj.sel.sendBtn).val('Send to IDF4 Logistics');
													obj.activateBtn('send');
													obj.updateData('stage=3');
												break;
												default:
													console.log('product sent to idf4');
													obj.updateMessage('Product already dispatched from IDF'+obj.idf+' to IDF 4', 'warn');
													obj.deactivateBtn();
												break;
											}
										} else {
											switch(data.ProductStage) {
												case "1":
													obj.updateMessage('Product dispatched from IDF' + data.idfID + ' production. Ready to be received.');
													$(obj.sel.receiveBtn).val('Receive from IDF' + data.idfID + ' Production');
													obj.activateBtn('receive');
													obj.updateData('stage=2');
												break;
												case "2":
										...
										obj.updateMessage('Product production location and your location does not match!', 'error');
										obj.deactivateBtn();
									}
								}
							}
