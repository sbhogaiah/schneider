...
if(isset($_POST['bayid']) && isset($_POST['bayname']) && isset($_POST['username']) && isset($_POST['serialnum']) && isset($_POST['modelid'])) {
	...
	$RunQueryPPI = mysqli_query($db,$GetPPI) or die('Cannot connect to PPI at pause test'); 
	...
	$RunUpdatePPI = mysqli_query($db, $UpdatePPI) or die("Could not update PPI at pause test!");
	...
	if($testToNow > $midToNow) { //overlap
		...
		$run_check_bayutil = mysqli_query($db, $check_bayutil) or die('Cannot connect to bay util table at pause test!');
		...
		if(mysqli_num_rows($run_check_bayutil) == 0) { // overlap and row does not exist
			...
			$RunQueryBayUTR = mysqli_query($db,$InsertBayUTR) or die("Connection was not created for InsertBayUTR at pause test!");
			...
			$UpdateBayUTbl = "UPDATE bayutil SET DownTime=StaticDownTime+CEIL(TIMESTAMPDIFF(MINUTE,DownStart,NOW())),
									...
									UsageEnd=NOW(), DownEnd=NOW(), DownStart=NULL, UsageStart=NULL,
									Status='up' 
									WHERE BayDate=CURDATE() AND BayActualName='$bayactual'";						
			$RunUpdateBayUTbl = mysqli_query($db,$UpdateBayUTbl) or die('Cannot connect to Bay Utilization at pause test!');
		}
	...
	$testeractivity_q = "UPDATE testeractivity SET TestStage='4', PausedUsageTime='$PPIUsageTime', PausedDownTime='$PPIDownTime', PausedStatus='$PPIStatus' WHERE TesterUsername = '$username'";
	$run_testeractivity_q = mysqli_query($db, $testeractivity_q) or die('Cannot connect to testeractivity at pause test!');
