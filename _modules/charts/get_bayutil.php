		...
		if(mysqli_num_rows($run_query)>0) {
			...
			while($row = mysqli_fetch_assoc($run_query)) {
				...
				if($row['Status'] == "up") {
					...
					$upd_query2 = "UPDATE bayutil SET UsageTime=StaticUsageTime+TIMESTAMPDIFF(MINUTE,UsageStart,NOW()) WHERE BayDate=CURDATE() AND BayActualName = '$row[BayActualName]'";
				...
				if($row['Status'] == "down") {
					...
					$upd_query2 = "UPDATE bayutil SET DownTime=StaticDownTime+TIMESTAMPDIFF(MINUTE,DownStart,NOW()) WHERE BayDate=CURDATE() AND BayActualName = '$row[BayActualName]'";
					$run_upd_query2 = mysqli_query($db, $upd_query2);
